      ...
      <div className="max-w-4xl mx-auto px-[5%] py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {PROJECTS.map((p) => (
            ...
            >
              <div className={`rounded-xl p-5 flex flex-col gap-3 h-full ${dark ? "bg-[#1a1a2e]" : "bg-white shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]"}`}>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {p.stack.map((tag) => (
                    <span key={tag} className={`font-mono text-xs px-2 py-0.5 rounded border ${dark ? "bg-white/5 border-white/10 text-gray-400" : "bg-slate-100 border-slate-200/80 text-slate-600"}`}>
                      {tag}
                    </span>
                  ))}
                </div>
